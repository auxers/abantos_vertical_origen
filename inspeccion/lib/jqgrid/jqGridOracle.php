<?php class jqGridDB
{
    ...
    public static function limit($sqlId, $dbtype, $nrows = - 1, $offset = - 1, $order = '', $sort = '')
    {
        ...
        if ($offset >= 0 && $nrows >= 0)
        {
            $psql = "SELECT z2.*				FROM(					SELECT z1.*, ROWNUM AS \"jqgrid_row\"					FROM(						" . $sqlId . "					) z1				) z2				WHERE z2.\"jqgrid_row\" BETWEEN " . ($offset + 1) . " AND " . ($offset + $nrows);
        }
        return $psql;
    }
    ...
    {
        if ($rs) return oci_num_fields($rs);
        else return 0;
    }
    ...
    public static function getPrimaryKey($table, $conn, $dbtype)
    {
        ...
        {
            $sql = "SELECT cols.table_name, cols.column_name, cols.position, cons.status, cons.owner" . " FROM all_constraints cons, all_cons_columns cols" . " WHERE cols.table_name = '" . $table . "'" . " AND cons.constraint_type = 'P'" . " AND cons.constraint_name = cols.constraint_name" . " AND cons.owner = cols.owner" . " ORDER BY cols.table_name, cols.position";
            $stmt = self::query($conn, $sql);
            if ($stmt)
            {
                $res = self::fetch_num($stmt);
