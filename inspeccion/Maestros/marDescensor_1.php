...
$grid->SelectCommand = 'SELECT * FROM MarcaDes';
$grid->table='MarcaDes';		  	   // set the table where add-del-edit data
...
$grid->setUrl('marDescensor_1.php');  // Set the url from where we obtain the data
...
$buttonoptions = array("#pager",
	array("caption"=>"", "buttonicon"=>"ui-icon-excel", "title"=>"Exportar a Excel", "onClickButton"=>"js: function(){
		$('#grid').jqGrid('excelExport',{tag:'excel', url:'marDescensor_1.php'});}"
	)
);
...
$buttonoptions = array("#pager",
	array("caption"=>"", "buttonicon"=>"ui-icon-pdf", "title"=>"Generar PDF", "onClickButton"=>"js: function(){
		$('#grid').jqGrid('excelExport',{tag:'pdf', url:'marDescensor_1.php'});}"
	)
);
$grid->callGridMethod("#grid", "navButtonAdd", $buttonoptions);
$grid->callGridMethod("#grid", "navSeparatorAdd",array("#pager"));

// Administrar PIE Certificados Descensor
$viewevent = <<< ONVIEW
function() {
	...
	if (rowid > 0)
	{
		...
		window.parent.$("#ModalHis").html("<iframe allowtransparency='allowtransparency' id='vent_info' src='Maestros/marDescensorPie_0.php?grupo="+rowid+"' width='100%' height='200' frameborder='0' scrolling='no'></iframe>");
...
else if ($oper == "add")
{
	...
	$grid->setAfterCrudAction($oper, "INSERT INTO LiteralPie (Tipo,Grupo,Texto) VALUES (5,".$IdTmp.",'')");
	$grid->setAfterCrudAction($oper, "INSERT INTO LiteralPie (Tipo,Grupo,Texto) VALUES (5,".$IdTmp.",'')");
	$grid->setAfterCrudAction($oper, "INSERT INTO LiteralPie (Tipo,Grupo,Texto) VALUES (5,".$IdTmp.",'')");
	$grid->setAfterCrudAction($oper, "INSERT INTO LiteralPie (Tipo,Grupo,Texto) VALUES (5,".$IdTmp.",'')");
}
...
{   // Buscamos y Borrarmos, los Literales de PIE	
	$grid->setAfterCrudAction($oper, "DELETE FROM LiteralPie WHERE Tipo=5 AND Grupo=".jqGridUtils::GetParam("Id"));
}
