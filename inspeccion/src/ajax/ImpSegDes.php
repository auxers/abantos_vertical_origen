...
$Query = "SELECT LC.Fecha, P.Nombre, L.NumeroTorre AS Torre, L.TipoAerogeneradorGAMESA AS TipoAEG, LC.NSerie, A.Nombre AS DesMarca, 
	LC.OT, LC.Estado FROM ListaCtrlDes LC JOIN Lineas L ON L.Id=LC.IdLinea JOIN Parques P ON P.Id=L.IdParque
	JOIN MarcaDes A ON A.Id = LC.Fabricante WHERE ";
...
if (($result = mysql_query($Query, $conn)))
{
	...
	{   // * Creamos PDF *
		...
		while($row = mysql_fetch_array($result))
		{
			...
			$pdf->Cell(65,5,$row['Nombre'],0,0,'L',false,'',true,20);
			$pdf->Cell(15,5,$row['Torre'],0,0,'C',false,'', true,5);
			$pdf->Cell(30,5,$row['TipoAEG'],0,0,'C',false,'', true,15);
			$pdf->Cell(30,5,$row['NSerie'],0,0,'C',false,'', true,20);
			$pdf->Cell(30,5,$row['DesMarca'],0,0,'C',false,'', true,20);
...
function fCabecera($ObjPDF)
{
	...
	$ObjPDF->Cell(285,5,"Seguimiento Diario de Descensores/Evacuadores",'1',0,'C');
	$ObjPDF->Ln(6.5);
	
	$ObjPDF->SetFont('Arial','B', 9);
	$ObjPDF->Cell(15,5,"Fecha",'B',0,'C');
	$ObjPDF->Cell(65,5,"Parque",'B',0,'C');
	$ObjPDF->Cell(15,5,"Torre",'B',0,'C');
	$ObjPDF->Cell(30,5,"Tipo AEG",'B',0,'C');
	$ObjPDF->Cell(30,5,"Nº Serie",'B',0,'C');
	$ObjPDF->Cell(30,5,"Fabricante",'B',0,'C');
